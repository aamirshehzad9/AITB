...
<#
.SYNOPSIS
    Install AITB Dashboard service to green staging slot using NSSM
    
.DESCRIPTION
    Episode 5 service installation script that:
    - Registers Windows service using NSSM
    - Points to versioned green slot deployment
    - Configures Python virtual environment with Streamlit
    - Configures logging to D:\logs\aitb\dashboard\
...
.EXAMPLE
    .\install-dashboard-green.ps1 -Version "1.0.0"
    
.EXAMPLE
    .\install-dashboard-green.ps1 -Version "1.0.0" -NSSMPath "C:\Tools\nssm\nssm.exe"
#>
...
# Configuration
$SERVICE_NAME = "AITB-Dashboard-Green"
$SERVICE_DISPLAY_NAME = "AITB Dashboard Service (Green Slot)"
$SERVICE_DESCRIPTION = "AI Trading Bot Streamlit dashboard service running in green staging slot"

$AITB_ROOT = "D:\apps\aitb"
$LOGS_ROOT = "D:\logs\aitb"
$GREEN_SLOT = "green"

$SERVICE_PATH = Join-Path $AITB_ROOT "dashboard\$GREEN_SLOT\$Version"
$PYTHON_EXE = Join-Path $SERVICE_PATH ".venv\Scripts\python.exe"
$STREAMLIT_EXE = Join-Path $SERVICE_PATH ".venv\Scripts\streamlit.exe"
$MAIN_SCRIPT = Join-Path $SERVICE_PATH "app.py"
$LOG_PATH = Join-Path $LOGS_ROOT "dashboard"
$STDOUT_LOG = Join-Path $LOG_PATH "dashboard-green-stdout.log"
$STDERR_LOG = Join-Path $LOG_PATH "dashboard-green-stderr.log"
...
function Install-DashboardService {
    ...
    Write-Log "Installing AITB Dashboard service using NSSM"
    
    try {
        # Install service with Streamlit command
        Write-Log "Installing service: $SERVICE_NAME"
        & $NSSMExe install $SERVICE_NAME $STREAMLIT_EXE run $MAIN_SCRIPT --server.port 8501 --server.address 0.0.0.0 --server.headless true
        if ($LASTEXITCODE -ne 0) {
        ...
        # Set environment variables for Dashboard service
        $envVars = @(
        ...
        & $NSSMExe set $SERVICE_NAME AppEnvironmentExtra ($envVars -join "`0")
...
# Main execution
Write-Log "Starting AITB Dashboard Green service installation"
...
if (-not (Test-Path $PYTHON_EXE)) {
    Write-Log "Python executable not found: $PYTHON_EXE" -Level "ERROR"
    Write-Log "Virtual environment may not be set up correctly" -Level "ERROR"
    exit 1
}

if (-not (Test-Path $STREAMLIT_EXE)) {
    Write-Log "Streamlit executable not found: $STREAMLIT_EXE" -Level "ERROR"
    Write-Log "Streamlit may not be installed in virtual environment" -Level "ERROR"
    exit 1
}

if (-not (Test-Path $MAIN_SCRIPT)) {
    Write-Log "Main script not found: $MAIN_SCRIPT" -Level "ERROR"
    exit 1
}
...
# Install service
$success = Install-DashboardService -NSSMExe $nssmExe

if ($success) {
    Write-Log "`n=== INSTALLATION SUMMARY ===" -Level "SUCCESS"
    Write-Log "Service Name: $SERVICE_NAME"
    Write-Log "Display Name: $SERVICE_DISPLAY_NAME"
    Write-Log "Streamlit Executable: $STREAMLIT_EXE"
    Write-Log "Main Script: $MAIN_SCRIPT"
    Write-Log "Working Directory: $SERVICE_PATH"
    Write-Log "Dashboard URL: http://localhost:8501"
