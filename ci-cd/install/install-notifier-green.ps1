...
<#
.SYNOPSIS
    Install AITB Notifier service to green staging slot using NSSM
...
.DESCRIPTION
    ...
    - Configures logging to D:\logs\aitb\notifier\
...
.EXAMPLE
    .\install-notifier-green.ps1 -Version "1.0.0"
    
.EXAMPLE
    .\install-notifier-green.ps1 -Version "1.0.0" -NSSMPath "C:\Tools\nssm\nssm.exe"
#>
...
# Configuration
$SERVICE_NAME = "AITB-Notifier-Green"
$SERVICE_DISPLAY_NAME = "AITB Notifier Service (Green Slot)"
$SERVICE_DESCRIPTION = "AI Trading Bot notification service running in green staging slot"

$AITB_ROOT = "D:\apps\aitb"
$LOGS_ROOT = "D:\logs\aitb"
$GREEN_SLOT = "green"

$SERVICE_PATH = Join-Path $AITB_ROOT "notifier\$GREEN_SLOT\$Version"
$PYTHON_EXE = Join-Path $SERVICE_PATH ".venv\Scripts\python.exe"
$MAIN_SCRIPT = Join-Path $SERVICE_PATH "main.py"
$LOG_PATH = Join-Path $LOGS_ROOT "notifier"
$STDOUT_LOG = Join-Path $LOG_PATH "notifier-green-stdout.log"
$STDERR_LOG = Join-Path $LOG_PATH "notifier-green-stderr.log"
...
function Install-NotifierService {
    param([string]$NSSMExe)
    
    Write-Log "Installing AITB Notifier service using NSSM"
    
    try {
        # Install service with Python executable and script
        Write-Log "Installing service: $SERVICE_NAME"
        & $NSSMExe install $SERVICE_NAME $PYTHON_EXE $MAIN_SCRIPT
        if ($LASTEXITCODE -ne 0) {
        ...
        # Set environment variables for Notifier service
        $envVars = @(
            "PYTHONPATH=$SERVICE_PATH",
            "PYTHONUNBUFFERED=1",
            "NOTIFIER_ENV=staging"
        )
...
# Main execution
Write-Log "Starting AITB Notifier Green service installation"
...
# Install service
$success = Install-NotifierService -NSSMExe $nssmExe
